...
\restrict CddnUiUznebq7PzOL8Alb4bAmNVEqzscrmrEc9XURU3m8NuuDv6869KPT9fwsXc
...
ALTER TABLE inver.answers ALTER COLUMN answer_id SET DEFAULT nextval('inver.answers_answer_id_seq');
...
ALTER TABLE inver.annotations ALTER COLUMN note_id SET DEFAULT nextval('inver.annotations_note_id_seq');
...
ALTER TABLE inver.class ALTER COLUMN class_id SET DEFAULT nextval('inver.class_class_id_seq');
...
ALTER TABLE inver.collection_items ALTER COLUMN collection_item_id SET DEFAULT nextval('inver.collection_items_collection_item_id_seq');
...
ALTER TABLE inver.collections ALTER COLUMN collection_id SET DEFAULT nextval('inver.collections_collection_id_seq');
...
ALTER TABLE inver.comments ALTER COLUMN comment_id SET DEFAULT nextval('inver.comments_comment_id_seq');
...
ALTER TABLE inver.courses ALTER COLUMN course_id SET DEFAULT nextval('inver.courses_course_id_seq');
...
ALTER TABLE inver.files ALTER COLUMN file_id SET DEFAULT nextval('inver.files_file_id_seq');
...
ALTER TABLE inver.questions ALTER COLUMN question_id SET DEFAULT nextval('inver.questions_question_id_seq');
...
ALTER TABLE inver.reactions ALTER COLUMN like_id SET DEFAULT nextval('inver.reactions_like_id_seq');
...
ALTER TABLE inver.test ALTER COLUMN test_id SET DEFAULT nextval('inver.test_test_id_seq');
...
ALTER TABLE inver.third_party_accounts ALTER COLUMN third_id SET DEFAULT nextval('inver.third_party_accounts_id_seq');
...
ALTER TABLE inver.users ALTER COLUMN user_id SET DEFAULT nextval('inver.users_user_id_seq');
...
ALTER TABLE inver.view_records ALTER COLUMN record_id SET DEFAULT nextval('inver.view_records_record_id_seq');
...
COPY inver.version (commit_id, version, update_info) FROM stdin;
019b751b-a2ad-73c6-970c-e561bcb11b5d	1.4.111.1000.alpha.1.aic	1. Switch to using the Postgres database; 2. Optimize data transmission and update logic.
019b1da5-6a40-7deb-89d3-c1f6040f828f	1.3.71.1000.alpha.4.aic	Added DOCX, XLSX, and PPT reader
019b2d26-adaf-72f8-bd47-e90776065c16	1.3.80.1000.alpha.7.aic	Added epub reader, pdf reader and text reader
019b5484-eb1a-7a90-bfb0-f86bfb3b6206	1.3.86.1116.beta.1.aic	Update some reader page's style design
019b5b05-92d8-7139-abbf-5248d7772f19	1.3.89.1132.beta.2.aic	Update and unify reader styles
019b6ac6-a8d1-71a9-adce-e5bffbbe1c72	1.3.97.1200.release.aic	1. Completely updated and unified the global page style; 2. Updated support for compiling C, C++, Python, and Rust code.
0a54bac6-9861-11f0-a41f-6b3c1ccfef93	1.1.4.1100.beta.2.aic	Fixed an error when the logged in user does not exist
1009d1c0-3c4e-11f0-8b9c-a51ae60b576a	1.1.0.1000.beta.1.aic	Update package name; Fix some bugs in history records; Add AiBot page.
...
2439cb90-3c90-11f0-a211-1b03c85e0ae4	1.1.4.1000.beta.2.aic	Added AiBot function to support exporting chat history; optimized AiBot page; adjusted AiBot Server logic.
...
2f2c0b4c-a367-11f0-a218-325096b39f47	1.2.51.1000.beta.1.aic	Added support for 23 languages ​including Simplified Chinese, French, German, etc.
49cfc210-d367-11f0-877c-752b2124c36f	1.3.63.1000.alpha.1.aic	1. Introduce the navigation bar designed by Waveflux; 2. Redesign the video playback page.
67cc5000-3b9e-11f0-8b9c-a51ae60b576a	1.0.34.1000.beta.2.aic	Fixed the bug that database automatic backup was not saved; Fixed the bug that users would get an error when directly turning pages on the browsing history page; Optimized the user history structure.
7aff4c00-3bbb-11f0-8b9c-a51ae60b576a	1.0.34.9000.beta.3.aic	Dynamically display the system version; Optimize user history storage; Display user browsing times, first and last browsing time; Fix some known bugs.
9bc4dd30-d32b-11f0-9dbd-d964415d14f3	1.2.59.100.release.aic	Adjusted audio player interface
9dd2f0a9-4349-11ef-acd1-005056c00001	1.0.22.5000.alpha.5	Adjusted web architecture
ac1f6117-9cfa-11f0-922a-e3cd01e5ca3b	1.1.7.1000.zeta.0.aic	Added multi-language options (currently only supports English); Fixed some known bugs.
cbd2af32-a37b-11f0-9865-325096b39f47	1.2.55.1000.beta.2.aic	Fixed errors caused by the navigation bar on some pages; Fixed the error that administrators could not modify or delete courses on the course page; Fixed the error that administrators jumped back to the course page after deleting a course on the search page; Adjusted the style of the language selection page.
e24e0920-3b9b-11f0-8b9c-a51ae60b576a	1.0.30.1000.beta.1.aic	Added automatic database backup for Linux servers
019b8331-2cdd-7eca-bbdc-fbc4ff135f06	1.4.128.1000.alpha.2.aic	1. Update the database schema and add some new tables; 2. Add course favorites to the default favorites folder.
\.
...
10000000061	10000000036	10000000000	0	2026-01-01 12:06:24.139378	32	2025-12-18 00:03:32
...
10000000051	10000000019	10000000000	0	2026-01-01 12:11:03.013198	8	2025-12-07 11:35:41
...
-- 根据现有数据更新序列的当前值
-- 注意：这些值应该根据实际插入的数据来调整
SELECT setval('inver.answers_answer_id_seq', COALESCE((SELECT MAX(answer_id) FROM inver.answers), 10000000000) + 1, false);
SELECT setval('inver.annotations_note_id_seq', COALESCE((SELECT MAX(note_id) FROM inver.annotations), 10000000000) + 1, false);
SELECT setval('inver.class_class_id_seq', COALESCE((SELECT MAX(class_id) FROM inver.class), 10000000000) + 1, false);
SELECT setval('inver.collection_items_collection_item_id_seq', COALESCE((SELECT MAX(collection_item_id) FROM inver.collection_items), 10000000000) + 1, false);
SELECT setval('inver.collections_collection_id_seq', COALESCE((SELECT MAX(collection_id) FROM inver.collections), 10000000000) + 1, false);
SELECT setval('inver.comments_comment_id_seq', COALESCE((SELECT MAX(comment_id) FROM inver.comments), 10000000000) + 1, false);
SELECT setval('inver.courses_course_id_seq', COALESCE((SELECT MAX(course_id) FROM inver.courses), 10000000000) + 1, false);
SELECT setval('inver.files_file_id_seq', COALESCE((SELECT MAX(file_id) FROM inver.files), 10000000000) + 1, false);
SELECT setval('inver.questions_question_id_seq', COALESCE((SELECT MAX(question_id) FROM inver.questions), 10000000000) + 1, false);
SELECT setval('inver.reactions_like_id_seq', COALESCE((SELECT MAX(like_id) FROM inver.reactions), 10000000000) + 1, false);
SELECT setval('inver.test_test_id_seq', COALESCE((SELECT MAX(test_id) FROM inver.test), 10000000000) + 1, false);
SELECT setval('inver.third_party_accounts_id_seq', COALESCE((SELECT MAX(third_id) FROM inver.third_party_accounts), 10000000000) + 1, false);
SELECT setval('inver.users_user_id_seq', COALESCE((SELECT MAX(user_id) FROM inver.users), 10000000000) + 1, false);
SELECT setval('inver.view_records_record_id_seq', COALESCE((SELECT MAX(record_id) FROM inver.view_records), 10000000000) + 1, false);
...
\unrestrict CddnUiUznebq7PzOL8Alb4bAmNVEqzscrmrEc9XURU3m8NuuDv6869KPT9fwsXc