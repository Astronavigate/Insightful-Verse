...
CREATE TABLE `annotations` (
                               ...
                               `note_type` enum('bookmark','highlight','note') NOT NULL,
                               `note_cfi` varchar(256) NOT NULL,
                               `note_content` varchar(256) DEFAULT NULL,
                               `note_color` varchar(28) DEFAULT '#ffd54f',
                               `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
                               `updated_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
                               PRIMARY KEY (`note_id`),
                               UNIQUE KEY `uk_user_book_type_cfi` (`user_id`,`book_id`,`note_type`,`note_cfi`),
...
CREATE TABLE `collection_items` (
                                    ...
                                    `item_type` enum('course','files') NOT NULL,
...
CREATE TABLE `comments` (
                            ...
                            `type` enum('course','files','collection') NOT NULL,
...
CREATE TABLE `file` (
                        ...
                        `file_remark` varchar(2000) DEFAULT NULL,
...
CREATE TABLE `reactions` (
                             ...
                             `user_id` bigint NOT NULL,
                             `content_type` enum('course','files','collection','comment') NOT NULL,
                             `related_id` bigint NOT NULL,
                             `action` enum('like','dislike') NOT NULL,
                             PRIMARY KEY (`like_id`),
                             UNIQUE KEY `user_content_action` (`user_id`,`content_type`,`related_id`,`action`)
);

-- stu_assignments
DROP TABLE IF EXISTS `stu_assignments`;
...
CREATE TABLE `test` (
                        ...
                        `test_introduction` longtext NOT NULL,
                        PRIMARY KEY (`test_id`),
                        KEY `course_id` (`course_id`)
);
...
CREATE TABLE `third_party_accounts` (
                                        ...
                                        `provider_type` enum('ravon','google','apple','microsoft','other') NOT NULL,
                                        PRIMARY KEY (`id`),
                                        KEY `user_id` (`user_id`)
);
...
-- collection_items 没有插入数据（保留空表）

-- collections 没有插入数据（原来依赖 users 的外键已移除）
...
-- view_records 数据
INSERT INTO `view_records` VALUES
...
-- version 数据（原 ivversion 内容已合并到 version）
...
-- stu_assignments 没有数据（保留空表）

-- question 没有数据（保留空表）

-- test 没有数据（保留空表）

-- third_party_accounts 没有数据（保留空表）

-- users 数据（列顺序与表一致）
INSERT INTO `users` VALUES
                        (10000000000,'Ravon Gonzales','ravongonzales@gmail.com','+14152836259','$argon2id$v=19$m=65536,t=3,p=1$55CmHLytIqlWdQguB+chGw$uHhLgR88RwGNhxu35KmFSQSfW2VwLotYWjHUtfdDEXI',NULL,'infinite','/image/system/avatar/default.png'),
                        (10000000001,'114','1078959112@qq.com','+8615640993693','$argon2id$v=19$m=65536,t=3,p=1$0nckdWH+mGAO1TtbWJQQuQ$U7S2iuL56DyaHYsyWdIIntqnsJenaWTACBQYhJwCgY0',NULL,'normal','/image/system/avatar/default.png'),
                        (10000000002,'长青','1243637340@qq.com','17347140977','$argon2id$v=19$m=65536,t=3,p=1$0nckdWH+mGAO1TtbWJQQuQ$U7S2iuL56DyaHYsyWdIIntqnsJenaWTACBQYhJwCgY0',NULL,'normal','/image/system/avatar/default.png'),
                        (10000000003,'Tester','Tester','0000000000','$argon2id$v=19$m=65536,t=3,p=1$mH60WgbcZmHsxhzaexLdbg$5qRcBGu1bdso0xgBXZT5e7N8EGKB4V0ua9lEu+0NEvw',NULL,'normal','/image/system/avatar/default.png'),
                        (10000000004,'Test','ivadmin@test.ravon.tech','+8618141192117','$argon2id$v=19$m=65536,t=3,p=1$OnrXIvvI9Lsu/nBm3DXjkw$U6ZpOa6d9yANMlzRM/5fLURw0Ylih7Dte+Au/f4grDc',NULL,'admin','/image/system/avatar/default.png');

-- 其余表如果你有更多 insert 我可以继续合并
