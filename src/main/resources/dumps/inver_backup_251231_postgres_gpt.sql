...
DROP SCHEMA IF EXISTS inver CASCADE;
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'note_type_enum') THEN
    CREATE TYPE note_type_enum AS ENUM ('bookmark','highlight','note');
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'item_type_enum') THEN
    CREATE TYPE item_type_enum AS ENUM ('course','files');
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'content_type_enum') THEN
    CREATE TYPE content_type_enum AS ENUM ('course','files','collection','comment');
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'provider_type_enum') THEN
    CREATE TYPE provider_type_enum AS ENUM ('ravon','google','apple','microsoft','other');
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'authority_type_enum') THEN
    CREATE TYPE authority_type_enum AS ENUM ('normal','teacher','admin','infinite');
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'action_type_enum') THEN
    CREATE TYPE action_type_enum AS ENUM ('like','dislike');
  END IF;
END $$;


-- General Trigger Function: Automatically Update Timestamp
...
-- Drop existing tables if they exist (safety; already dropped by schema CASCADE above,
-- but keep for clarity if you reuse without DROP SCHEMA)
...
CREATE TABLE annotations (
  note_id BIGINT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
  ...
  CONSTRAINT uk_user_book_type_cfi UNIQUE (user_id, book_id, note_type, note_cfi)
);
CREATE INDEX idx_annotations_user_book ON annotations(user_id, book_id);

-- trigger for annotations.updated_at
...
CREATE TABLE collection_items (
  collection_item_id BIGINT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
...
CREATE TABLE collections (
  collection_id BIGINT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
  user_id BIGINT NOT NULL,
...
CREATE INDEX idx_file_course_id ON files(course_id);
CREATE INDEX idx_file_upload_user ON files(upload_user);
...
CREATE INDEX idx_view_records_file_id ON view_records(file_id);
...
CREATE TABLE reactions (
  ...
  CONSTRAINT uq_user_content_action UNIQUE (user_id, content_type, related_id, action)
);
...
CREATE INDEX idx_stu_assignments_test_id ON stu_assignments(test_id);
CREATE INDEX idx_stu_assignments_submitter_id ON stu_assignments(submitter_id);
...
CREATE INDEX idx_question_submitter_id ON question(submitter_id);

CREATE TABLE "test" (
  test_id BIGINT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
  test_name VARCHAR(100) NOT NULL,
  course_id BIGINT NOT NULL,
  test_introduction TEXT NOT NULL
);
CREATE INDEX idx_test_course_id ON "test"(course_id);
...
CREATE TABLE users (
  ...
  class_id BIGINT,
  authority authority_type_enum NOT NULL,
...
INSERT INTO files (file_id,file_name,file_type,course_id,file_remark,upload_user,upload_date,file_path) VALUES
...
INSERT INTO users (user_id,username,email,phone,password,class_id,authority,avatar_url) VALUES
